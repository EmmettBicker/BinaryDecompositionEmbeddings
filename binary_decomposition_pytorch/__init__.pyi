...
class BinaryDecompositionEmbedding(nn.Linear):
    r"""Modification of nn.Linear. Pass in a tensor and it gets converted to
        its binary index representation. The binary index is normalized so 0
        becomes -1 and 1 remains as 1. Then, the binary index representation is
        passed through a linear layer with log2(num_embeddings) inputs and
        hidden_size output.
